...
export default function VertexGripsLayer({
  model,
  mode,
  mapToScreen,
  activeGeozone,
}: VertexGripsLayerProps) {
  if (mode !== 'VIEW' || !activeGeozone) return null;

  return (
    <g id="svg_cad_grips" className="pointer-events-none">
      {/* Building vertex grips (small red squares) */}
      {activeGeozone.type === 'building' &&
        model.buildings
          .filter((b) => b.id === activeGeozone.id)
          .map((b) =>
            b.points.map((pt, idx) => {
      ...
      {activeGeozone.type === 'restriction' &&
        model.restrictions
          .filter((r) => r.id === activeGeozone.id)
          .map((r) =>
            r.points.map((pt, idx) => {
              const { u, v } = mapToScreen(pt.x, pt.y);
              return (
      ...
      {activeGeozone.type === 'land_use' &&
        (model.landUseExplication || [])
          .filter((lu) => lu.id === activeGeozone.id)
          .map((lu) =>
